...
def show_login_page():
    ...
    with col2:
        # Check if OAuth is configured before trying to get auth URL
        if hasattr(google_oauth, 'is_configured') and google_oauth.is_configured:
            try:
                ...
                st.warning(f"⚠️ OAuth error: {str(e)}")
                show_skip_auth_option()
        else:
            st.warning("⚠️ OAuth not configured")
            show_skip_auth_option()

def show_skip_auth_option():
    """Show option to skip authentication"""
    st.info("""
    **OAuth is not configured. You can:**
    ...
    2. **Set up Google OAuth** by following these steps:
       ...
       - Update your Streamlit secrets with real credentials
    """)
